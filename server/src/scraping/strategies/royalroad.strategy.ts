...
@Injectable()
class RoyalRoadStrategy implements ScrapingStrategy {
    constructor(@InjectModel(Book.name) private bookModel: Model<BookDocument>) {}

    async scrape(pages = 2): Promise<void> {
        this.crawlTopRated(pages);
    }
    ...
    async getSimilarFictions(fictionId: string, numberOfSimilarFictions: number = 3): Promise<Types.ObjectId[]> {
        ...
        const ids: Types.ObjectId[] = existingBooks.map(book => book._id as Types.ObjectId);

        return ids;
    }
}

export { RoyalRoadStrategy };